...
class EventDetector(nn.Module):
    def __init__(self, pretrain, width_mult, lstm_layers, lstm_hidden, bidirectional=True, dropout=True):
        ...
        net = mobilenet_v2(pretrained=pretrain)
    ...
    def init_hidden(self, batch_size):
        ...
        if self.bidirectional:
            h0 = torch.zeros(2 * self.lstm_layers, batch_size, self.lstm_hidden, device=device)
            c0 = torch.zeros(2 * self.lstm_layers, batch_size, self.lstm_hidden, device=device)
        else:
        ...
        return (
            Variable(h0, requires_grad=True),
            Variable(c0, requires_grad=True),
        )
